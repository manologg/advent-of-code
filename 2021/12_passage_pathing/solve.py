...
def navigate(paths, current='start', visited=None):
    if not visited:
        visited = []
    ...
    if current == 'end':
        print(f'Path: {visited + [current]}')
        return 1

    amount_of_paths = 0
    for cave in paths[current]:
        amount_of_paths += navigate(paths, current=cave, visited=visited + [current])
...
print()
amount_of_paths = navigate(paths)
print(f'Amount of possible paths through the cave system: {amount_of_paths}')